...
namespace zlpanel {
    ...
    void FFTPanel::runFFT(const juce::Thread& thread) {
        ...
        if (std::abs(c_sample_rate_ - sample_rate) > 0.1) {
            ...
            const auto middle_fft_order = static_cast<int>(zlp::getMediumFFTOrder(sample_rate));
            ...
            if (fft_smooth_type_idx == 0) {
                for (auto& smoother : smoothers_) {
                    smoother.setSmooth(
                        ...
                        sample_rate, zldsp::analyzer::SpectrumSmoother::SmoothMethod::kOCT);
                }
            } else {
                for (auto& smoother : smoothers_) {
                    smoother.setSmooth(
                        zlstate::PFFTSmoothERBValue::kValues[static_cast<size_t>(fft_smooth_erb_value_idx)],
                        sample_rate, zldsp::analyzer::SpectrumSmoother::SmoothMethod::kERB);
                }
            }
        }
        // update xs para
