...
namespace zlpanel {
    ...
    private:
        ...
        [[maybe_unused]] float c_fft_min_db_{0.f};

        std::atomic<float> &fft_speed_idx_ref_;
        [[maybe_unused]] int fft_speed_idx_{zlstate::PFFTSpeed::kDefaultI};

        std::atomic<float> &fft_tilt_idx_ref_;
        [[maybe_unused]] int fft_tilt_idx_{zlstate::PFFTTilt::kDefaultI};
        ...
        [[maybe_unused]] int fft_smooth_oct_value_idx_{-1};
        [[maybe_unused]] int fft_smooth_erb_value_idx_{-1};
        [[maybe_unused]] int fft_smooth_type_idx_{-1};

        bool skip_next_repaint_{false};

        std::vector<float> xs_{}, ys_{};
        std::vector<float> frequencies_{};
        std::array<zlchore::thread::TriBuffer<juce::Path>, kNumSources> paths_;

        [[maybe_unused]] double c_sample_rate_{0.0};
        [[maybe_unused]] int history_size_{0};
        [[maybe_unused]] size_t num_point_{0};

        std::atomic<float> width_{0.f}, height_{0.f};
        std::atomic<float> font_size_{0.1f};
        [[maybe_unused]] float c_width_{0.f};
        [[maybe_unused]] float c_height_{0.f};
        [[maybe_unused]] float y_k_{0.f}, y_b_{0.f};
